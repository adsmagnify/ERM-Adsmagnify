-- Weekly offs: Sunday always. 1st, 3rd, 5th Saturday off.
-- 2nd and 4th Saturday are working days.
...
begin
  ...
  v_dow := extract(dow from in_local)::int;

  if v_dow = 0
     or (
       v_dow = 6
       and ((extract(day from in_local)::int - 1) / 7) + 1 in (1, 3, 5)
     )
  then
    status := 'Full day';
    status_reason := null;
